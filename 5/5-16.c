...
#define isbadchar(x) (!isalpha(x) && !isdigit(x) && !isspace(x))
...
int main(int argc, char *argv[])
{
	...
	if ((nlines = readlines(lineptr, MAXLINES)) >= 0)
	{
		if (numeric)
			...
			myqsort((void **) lineptr, 0, nlines - 1,
				  (int (*)(void *, void *))(strcmp_d));		/* directory order */
		else
			myqsort((void **) lineptr, 0, nlines - 1,
...
/* strcmp_d: compare s1 and s2 (directory flag) */
int strcmp_d(char *s1, char *s2, int fold)
{
	while (isbadchar(*s1))
		s1++;
	while (isbadchar(*s2))
	...
	while (fold ? (toupper(*s1) == toupper(*s2)) : (*s1 == *s2))
	{
		if (*s1 == '\0')
			return 0;
