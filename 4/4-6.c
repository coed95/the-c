...
#define		MAXVARS		64			/* numbers of variables that can be created */	
#define		NUMBER		'0'			/* signal that a number was found */
#define		ID			'1'			/* identifier for variables and functions */
#define		END			'2'			/* end of the string */
...
void checkid(char []);				/* check the passed identifier */
void makevar(char []);				/* create a new variable */
	...
	while ((type = getop(s)) != EOF)
	{
		switch (type) {
			...
			case '=':
				...
				var[pos].value = pop();
				last_var.value = var[pos].value;
				push(last_var.value);

				break;

			/* print last variable assigned */
			case '<':
...
void makevar(char s[])
{
	...
	while (var[i].name[0] != '\0' && i < MAXVARS - 1)
	{
		...
		{
			strncpy(last_var.name, s, MAX_ID_LEN);
			last_var.value = var[i].value;
	...
	strncpy(var[i].name, s, MAX_ID_LEN);
	strncpy(last_var.name, s, MAX_ID_LEN);
